...
로컬에서 실행 중인 Hermes 에이전트에 cron 스케쥴을 등록하여, 매주 월~금 오전 9시(KST)에 자동으로 데이터를 수집·계산하고 Slack `#morning-briefing` 채널(ID: C0B5Z8XUV41)로 포맷된 브리핑 메시지를 발송한다. LLM은 로컬 Ollama `gemma4:e4b` 모델을 사용한다. Gemini free tier의 분당 5회 요청 제한으로 인한 429 오류를 방지하고, 외부 API 의존성을 제거한다.
...
14. 시스템 관리자로서, LLM을 외부 API가 아닌 로컬 Ollama 모델로 실행하고 싶다. quota 제한 없이 안정적으로 브리핑을 발송하기 위해.
15. 시스템 관리자로서, Ollama가 Mac 부팅 시 자동 시작되길 원한다. cron 실행 시각에 항상 Ollama가 켜져 있도록 보장하기 위해.
...
### LLM 프로바이더: Ollama (로컬)
- Hermes `provider: custom` 사용. `OPENAI_BASE_URL=http://host.docker.internal:11434/v1` 로 Ollama OpenAI-compatible 엔드포인트 연결.
- `OPENAI_API_KEY=ollama` 설정 (Ollama는 실제 key 불필요, Hermes 빈값 거부 방지용 placeholder).
- 모델: `gemma4:e4b` (8B 파라미터, tool calling 지원 확인됨).
- Gemini API key는 `.env`에 유지하나 custom 프로바이더 사용 시 무시됨.
- Mac Docker는 `host.docker.internal`이 호스트 IP로 자동 해석됨. 추가 네트워크 설정 불필요.

### Ollama 자동시작
- Ollama 설치: Ollama.app (`/Applications/Ollama.app`). brew 설치가 아니므로 `brew services` 불가.
- 자동시작: Ollama.app 메뉴바 → "Start at Login" 활성화.
- 주의: Ollama가 꺼져 있으면 Hermes 전체 LLM 응답 불가.

### Docker 인프라
- `docker-compose.yml` 변경 없음.
- 환경변수·config 변경 후 컨테이너 재시작 필요 (환경변수 반영).
...
- `gemma4:e2b`와 `gemma4:e4b` 성능 비교 테스트.
- Gemini를 fallback으로 유지하는 이중 프로바이더 구성.
- Ollama 모델 업그레이드 자동화.
- 주말 실행 또는 공휴일 인식 로직.
...
- LLM: `gemma4:e4b` (Ollama, 로컬). tool calling 지원 확인됨 (Ollama capabilities: `tools`).
- Ollama가 꺼져 있으면 Hermes 전체 작동 불가. Ollama.app Start at Login 설정 필수.
